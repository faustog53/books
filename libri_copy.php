...
if( isset($_POST['btnadd']) )
{
  ...
  /*
    ...
    $titolo_isbdm      = test_input($_POST['titolo_isbdm']);
    $titolo_originale  = test_input($_POST['titolo_originale']);
    $titolo_uniforme   = test_input(SubStr($_POST['titolo_uniforme'],0,255));
    $editore           = test_input($_POST['editore']);
    $posizione         = StrToUpper(test_input($_POST['posizione']));   // 31/12/2020
	  $sezione           = StrToUpper(test_input($_POST['sezione']));   	// 31/12/2020
	  $gruppo            = StrToUpper(test_input($_POST['gruppo']));   	// 31/12/2020
    ...
    $testi_copertine = filter_var(test_input($_POST['testi_copertine']), FILTER_SANITIZE_STRING);
    $id              = $_POST['id'];

    $insert = "insert into $libri (titolo_isbdm, titolo_originale, titolo_uniforme, copertina_testo, editore, anno, collezione, collezione_nr, pubblicazione_tipo, pubblicazione_url, recensione_url, pagine, isbn, altezza, sbn_url, copertina, dorso, note, posizione, sezione, gruppo, libreria) 
    values(\"$titolo_isbdm\", \"$titolo_originale\",\"$titolo_uniforme\",\"$testi_copertine\",'$editore','$anno','$collezione','$collezione_nr','$pubblicazione_tipo','$pubblicazione_url','$recensione_url', '$pagine','$isbn','$altezza','$url','$copertina', '$dorso', '$note',  '$posizione', '$sezione', '$gruppo', '$libreria')";
...
if( isset($_GET['id']) ){
  ...
  $titolo_isbdm = $titolo_originale = $collezione = $editore = $anno = $pagine = $altezza = $isbn = $url = $copertina = $dorso = $note = "";
}
?> 
...
$titolo_isbdmErr = $titolo_originaleErr = $collezioneErr = $editoreErr = $annoErr = $pagineErr = $altezzaErr = $isbnErr = $urlErr = $copertinaErr = $dorsoErr = $noteErr = "";
