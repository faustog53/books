...
$result = $conn->query("describe test.libri");

if ($result->num_rows > 0) {
    // output data of each row
    while ($row = $result->fetch_assoc()) {
        if ($row["Field"] != "id") {
            $form .= "<fieldset><label for='" . $row["Field"] . "'>" . $row["Field"] . "</label> <input type='text' name='" . $row["Field"] . "' size='100' value='".$row_rs_libri[$row["Field"]]."' ></fieldset>";
        }
    }
